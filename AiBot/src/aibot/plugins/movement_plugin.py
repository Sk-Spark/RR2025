...
class MovementControlPlugin:
    ...
    @kernel_function(
        description="Move the robot forward at a specified speed for a specified duration",
        name="move_forward"
    )
    async def move_forward(
        self, 
        speed: Annotated[int, "Speed percentage (0-100)"] = 50,
        duration: Annotated[float, "Duration in seconds (default 1.0)"] = 1.0
    ) -> Annotated[str, "Result of moving forward"]:
        """Move the robot forward for specified duration then auto-stop."""
        try:
            speed = max(0, min(100, int(speed)))  # Clamp speed between 0-100
            duration = max(0.1, min(10.0, float(duration)))  # Clamp duration between 0.1-10 seconds
    ...
    @kernel_function(
        description="Move the robot backward at a specified speed for a specified duration",
        name="move_backward"
    )
    async def move_backward(
        ...
        speed: Annotated[int, "Speed percentage (0-100)"] = 50,
        duration: Annotated[float, "Duration in seconds (default 1.0)"] = 1.0
    ) -> Annotated[str, "Result of moving backward"]:
        """Move the robot backward for specified duration then auto-stop."""
        try:
            speed = max(0, min(100, int(speed)))  # Clamp speed between 0-100
            duration = max(0.1, min(10.0, float(duration)))  # Clamp duration between 0.1-10 seconds
            success = await self.movement_controller.move_backward(speed, duration=duration)
            
            if success:
                return f"Robot moved backward at {speed}% speed for {duration} seconds and stopped automatically"
            else:
    ...
    @kernel_function(
        description="Turn the robot left for a specified duration",
        name="turn_left"
    )
    async def turn_left(
        self, 
        speed: Annotated[int, "Speed percentage (0-100)"] = 50,
        duration: Annotated[float, "Duration in seconds (default 1.0)"] = 1.0
    ) -> Annotated[str, "Result of turning left"]:
        """Turn the robot left for specified duration then auto-stop."""
        try:
            speed = max(0, min(100, int(speed)))  # Clamp speed between 0-100
            duration = max(0.1, min(10.0, float(duration)))  # Clamp duration between 0.1-10 seconds
            success = await self.movement_controller.turn_left(speed, duration=duration)
            
            if success:
                return f"Robot turned left at {speed}% speed for {duration} seconds and stopped automatically"
            else:
    ...
    @kernel_function(
        description="Turn the robot right for a specified duration",
        name="turn_right"
    )
    async def turn_right(
        ...
        speed: Annotated[int, "Speed percentage (0-100)"] = 50,
        duration: Annotated[float, "Duration in seconds (default 1.0)"] = 1.0
    ) -> Annotated[str, "Result of turning right"]:
        """Turn the robot right for specified duration then auto-stop."""
        try:
            speed = max(0, min(100, int(speed)))  # Clamp speed between 0-100
            duration = max(0.1, min(10.0, float(duration)))  # Clamp duration between 0.1-10 seconds
            success = await self.movement_controller.turn_right(speed, duration=duration)
            
            if success:
                return f"Robot turned right at {speed}% speed for {duration} seconds and stopped automatically"
            else:
