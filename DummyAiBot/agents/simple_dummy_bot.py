...
class DummyAiBot:
    ...
    async def _handle_terminal_movement(self, task_id: str, command: str):
        ...
        if result['success']:
            print(f"✅ Movement completed: {result['action']}")
            print(f"📍 New position: {result['final_position']}")
        else:
    ...
    async def _handle_terminal_camera(self, task_id: str, command: str):
        ...
        if result['success']:
            print(f"✅ Camera action completed: {result['action']}")
    ...
    async def _handle_terminal_scan(self, task_id: str, command: str):
        ...
        if result['success']:
            print(f"✅ Scan completed: {result['scan_type']}")
            print(f"📊 Scan points: {len(result['scan_results'])}")
        else:
            print(f"❌ Scan failed")
    ...
    async def _handle_terminal_patrol(self, task_id: str, command: str):
        ...
        print(f"🚶 Starting patrol with {points} points...")
        result = await self._execute_patrol_task(command, parameters)
        
        if result['success']:
            print(f"✅ Patrol completed: {points} points")
            print(f"📋 Patrol log: {len(result['patrol_log'])} entries")
        else:
    ...
    async def _handle_terminal_natural_language(self, task_id: str, command: str):
        ...
        if not task_analysis or task_analysis.get('task_type') == 'unknown':
            print("❌ Could not understand the command. Try being more specific or use 'help'.")
            return
        
        print(f"🔍 LLM Analysis: {task_analysis.get('task_type')} task")
        print(f"💭 Understanding: {task_analysis.get('description', 'Processing...')}")
        ...
        if result['success']:
            ...
            print(f"📋 Actions taken: {', '.join(result.get('executed_actions', []))}")
        else:
            print(f"❌ Task failed")
